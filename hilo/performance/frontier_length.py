...
def plot_data(data, savepath):
    ...
    hv_ax, overlay_ax = axs
        
    frontier_lengths = np.abs(data['ws'][:, 0, 0] - data['ws'][:, 1, 0])
    end_hvs = data['hvs'][:, -1]
    end_overlays = data['overlays'][:, -1]
    
    idxs = np.argsort(frontier_lengths)
    hv_ax.scatter(frontier_lengths, end_hvs)
    overlay_ax.scatter(frontier_lengths, end_overlays)

    # plot_panel(hv_ax, frontier_lengths[idxs], end_hvs[idxs], 'Groundtruth Hypervolume', 'Hypervolume', 'tab:blue')
    # plot_panel(overlay_ax, frontier_lengths[idxs], end_overlays[idxs], 'Pareto Overlay', 'Fraction correct', 'tab:green')
...
def parse_args():
    ...
    parser.add_argument(
        '--fig', type=Path, default=Path('hilo/output/pareto_length_vs_performance.pdf'),
