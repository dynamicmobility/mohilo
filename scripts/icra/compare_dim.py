...
def parse_args():
    p = argparse.ArgumentParser(description='NSGA-II and MOBO across action '
                                            'dimensions, one subdirectory each')
    p.add_argument('--dim', type=int, nargs='+', required=True,
                   help='action dimensions, e.g. --dim 1 2 3 4')
    p.add_argument('--output', type=Path, default=OUTPUT_DIR,
                   help='the ablation directory to write dim<d>/ into; '
